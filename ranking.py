...
def Reranking(data, MODEL_FOLDER="Magolor/deepex-ranking-model", batch_size=4, device='cuda'):
    ...
    with torch.no_grad():
        for key, values in data[0].items():
            ...
            for i, value in enumerate(values):
                if i == 0:
                    sentence = value
                    sorted_data[key]['sentence'] = sentence
                    sorted_data[key]['triples'] = []
                else:
                    text_triple = sentence+"[SEP] "+ value[0] + " " + value[2] + " " + value[1]
                    text_vector, triple_vector = model(text_triple)
                    text_vector, triple_vector = text_vector.detach().cpu().numpy(), triple_vector.detach().cpu().numpy()
                    contrastive_dis = float(np.linalg.norm(text_vector -triple_vector))
                    sorted_data[key]['triples'].append([value[0], value[2], value[1], contrastive_dis])
            sorted_data[key]['triples'] = sorted(sorted_data[key]['triples'], key=lambda x: x[-1])
    return sorted_data

if __name__ == "__main__":
#    json_file_path = "srl_aware_oie_test.json"
#    json_file_path = "CaRB_srl_aware_oie_test.json"
    json_file_path = "NYT_srl_aware_oie_test.json"
    with open(json_file_path, 'r') as file:
        result = json.load(file)
    data = Reranking(result)
    evaluation_file = 'tab_separated_' + json_file_path
